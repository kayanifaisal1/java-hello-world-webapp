pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                withCredentials([azureKeyVaultSecret(credentialsId: "${AZURE_CREDENTIALS_ID}", secretName: "${AZURE_SECRET_NAME}", vaultName: "${AZURE_KEYVAULT_NAME}", variable: 'SECRET_VALUE')]) {
